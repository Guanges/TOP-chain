...
bool xtop_evm_erc20_sys_contract::execute(xbytes_t input,
    ...
    case method_id_transfer_from: {
        ...
        if (!ec) {
            ...
            log.topics.push_back(top::evm_common::h256(top::to_string(evm_common::fromHex("0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef", evm_common::WhenError::Throw))));
            assert(log.topics.back().size == 32);
            log.topics.push_back(top::evm_common::h256(top::to_string(owner_address.to_h256())));
            assert(log.topics.back().size == 32);
            log.topics.push_back(top::evm_common::h256(top::to_string(recipient_address.to_h256())));
            assert(log.topics.back().size == 32);
    ...
    case method_id_approve: {
        ...
        if (!ec) {
            ...
            assert(log.data.size() == 32);
            log.topics.push_back(top::evm_common::h256(top::to_string(evm_common::fromHex("0x8c5be1e5ebec7d5bd14f71427d1e84f3dd0314c0f7b2291e5b200ac8c7c3b925", evm_common::WhenError::Throw))));
            assert(log.topics.back().size == 32);
            log.topics.push_back(top::evm_common::h256(top::to_string(caller_address.to_h256())));
            assert(log.topics.back().size == 32);
            log.topics.push_back(top::evm_common::h256(top::to_string(spender_address.to_h256())));
            assert(log.topics.back().size == 32);
    ...
    case method_id_mint: {
        ...
        if (!ec) {
            ...
            log.topics.push_back(top::evm_common::h256(top::to_string(evm_common::fromHex("0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef", evm_common::WhenError::Throw))));
            assert(log.topics.back().size == 32);
            log.topics.push_back(top::evm_common::h256(top::to_string(common::xeth_address_t::zero().to_h256())));
            assert(log.topics.back().size == 32);
            log.topics.push_back(top::evm_common::h256(top::to_string(recipient_address.to_h256())));
            assert(log.topics.back().size == 32);
