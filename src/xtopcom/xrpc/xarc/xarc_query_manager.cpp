...
#define ARC_REGISTER_V1_METHOD(func_name)                                                                                                                                      \
    m_query_method_map.emplace(std::pair<std::string, query_method_handler>{std::string{#func_name}, std::bind(&xarc_query_manager::func_name, this, _1)})

xarc_query_manager::xarc_query_manager(observer_ptr<store::xstore_face_t> store,
  ...
  : m_store(store), m_block_store(block_store), m_txpool_service(txpool_service), m_bh(m_store.get(), m_block_store.get(), nullptr), m_transaction_cache(transaction_cache) {
    ...
    ARC_REGISTER_V1_METHOD(queryNodeInfo);
    ARC_REGISTER_V1_METHOD(getElectInfo);
    ARC_REGISTER_V1_METHOD(queryNodeReward);
    ARC_REGISTER_V1_METHOD(listVoteUsed);
    ARC_REGISTER_V1_METHOD(queryVoterDividend);
    ARC_REGISTER_V1_METHOD(queryProposal);
    ARC_REGISTER_V1_METHOD(getStandbys);
    ARC_REGISTER_V1_METHOD(getCGP);
    ARC_REGISTER_V1_METHOD(getLatestTables);
}

void xarc_query_manager::call_method(xjson_proc_t & json_proc) {
...
void xarc_query_manager::getAccount(xjson_proc_t & json_proc) {
...
std::string xarc_query_manager::tx_exec_status_to_str(uint8_t exec_status) {
...
void xarc_query_manager::getTransaction(xjson_proc_t & json_proc) {
    const string & account = json_proc.m_request_json["params"]["account_addr"].asString();
...
void xarc_query_manager::getBlock(xjson_proc_t & json_proc) {
    ...
    if (type == "height") {
        ...
        xdbg("height: %llu", hi);
        auto vb = m_block_store->load_block_object(_owner_vaddress, hi, base::enum_xvblock_flag_committed, true, metrics::blockstore_access_from_rpc_get_block);
...
void xarc_query_manager::getChainInfo(xjson_proc_t & json_proc) {
    xJson::Value jv;
...
void xarc_query_manager::getIssuanceDetail(xjson_proc_t & json_proc) {
    auto get_zec_workload_map = [&](observer_ptr<store::xstore_face_t> store,
        ...
        if (store->get_map_property(contract_address.value(), height - 1, property_name, workloads) != 0) {
            xwarn("[xarc_query_manager::getIssuanceDetail] get_zec_workload_map contract_address: %s, height: %llu, property_name: %s",
        ...
        xdbg("[xarc_query_manager::getIssuanceDetail] get_zec_workload_map contract_address: %s, height: %llu, property_name: %s, workloads size: %d",
                contract_address.value().c_str(),
                height,
                property_name.c_str(),
                workloads.size());
    ...
    if (height == 0) {
        xwarn("[xarc_query_manager::getIssuanceDetail] height: %llu", height);
        return;
    }
    ...
    if (m_store->get_string_property(sys_contract_zec_reward_addr, height, xstake::XPROPERTY_REWARD_DETAIL, xissue_detail_str) != 0) {
        xwarn("[xarc_query_manager::getIssuanceDetail] contract_address: %s, height: %llu, property_name: %s",
    ...
    xdbg("[xarc_query_manager::getIssuanceDetail] reward contract height: %llu, onchain_timer_round: %llu, m_zec_vote_contract_height: %llu, "
...
void xarc_query_manager::getTimerInfo(xjson_proc_t & json_proc) {
...
void xarc_query_manager::queryNodeInfo(xjson_proc_t & json_proc) {
    std::string owner = json_proc.m_request_json["params"]["account_addr"].asString();
...
void xarc_query_manager::getElectInfo(xjson_proc_t & json_proc) {
    std::string owner = json_proc.m_request_json["params"]["account_addr"].asString();
    std::string target = json_proc.m_request_json["params"]["node_account_addr"].asString();
    xdbg("account: %s, target: %s", owner.c_str(), target.c_str());
...
void xarc_query_manager::set_sharding_vote_prop(xjson_proc_t & json_proc, std::string & prop_name) {
    std::string owner = json_proc.m_request_json["params"]["account_addr"].asString();
    std::string target = json_proc.m_request_json["params"]["node_account_addr"].asString();
...
void xarc_query_manager::set_sharding_reward_claiming_prop(xjson_proc_t & json_proc, std::string & prop_name) {
    std::string owner = json_proc.m_request_json["params"]["account_addr"].asString();
    std::string target = json_proc.m_request_json["params"]["node_account_addr"].asString();
...
void xarc_query_manager::queryNodeReward(xjson_proc_t & json_proc) {
...
void xarc_query_manager::getStandbys(xjson_proc_t & json_proc) {
    std::string target = json_proc.m_request_json["params"]["node_account_addr"].asString();
...
void xarc_query_manager::getCGP(xjson_proc_t & json_proc) {
    xJson::Value jv;
...
void xarc_query_manager::getLatestTables(xjson_proc_t & json_proc) {
    std::string owner = json_proc.m_request_json["params"]["account_addr"].asString();
